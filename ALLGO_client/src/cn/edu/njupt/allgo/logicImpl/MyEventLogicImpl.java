...
public class MyEventLogicImpl implements MyEventLogic {
	...
	public MyEventLogicImpl(Context context , RefreshInterFace refresh) {
	...
	public void getEvent(int page, int pagenum) {
        ...
        NetUtil netUtil = new NetUtil("event/user_timeline", refresh, context, new NetCallBack(){
			@Override
			public void getResult(JSONObject jsonObject) {
				try {
					 if(jsonObject.getString("response").equals("user_timeline")){
							 if(page1 == 1){
								 refresh.refresh(JSON.parseArray(jsonObject.getString("user_timeline") , EventVo.class) , 2);
							 }else{
								 refresh.refresh(JSON.parseArray(jsonObject.getString("user_timeline") , EventVo.class) , 3);
							 }
         	        }else{
